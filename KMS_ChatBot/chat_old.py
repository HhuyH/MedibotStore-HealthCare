    ...
    async def event_generator():
        ...
        for step in pipeline:
            ...
            if step == "symptom_extract":
                ...
                # Nếu không có triệu chứng mới → dùng triệu chứng đã lưu trước đó
                if not symptoms and stored_symptoms:
                    symptoms = stored_symptoms
                    logger.info("🔁 Không có triệu chứng mới → dùng stored_symptoms")

                # Nếu vẫn không có gì → fallback hoặc gợi mở
                if not symptoms:
                    if suggestion:
                        logger.info(f"🤖 GPT gợi ý câu hỏi vì không rõ triệu chứng: {suggestion}")
                ...
                # Kiểm tra có triệu chứng mới không
                ...
                if not unasked_symptoms:
                    logger.info("✅ Tất cả triệu chứng đã hỏi follow-up.")

                    # Kiểm tra đã hỏi triệu chứng liên quan chưa
                    ...
                    # Nếu đã hỏi rồi mà không còn gì thêm → chẩn đoán nếu đủ điều kiện
                    if intent in ("symptom_query", "general_chat"):
                        can_diagnose = await should_trigger_diagnosis(msg.message, symptoms, recent_messages)
                        logger.info(f"🔍 should_trigger_diagnosis(...) trả về: {can_diagnose}")
                    ...
                    # Fallback cuối nếu không đủ điều kiện chẩn đoán
                ...
                # Nếu còn triệu chứng chưa hỏi follow-up → hỏi tiếp
